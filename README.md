...
La solución propuesta busca responder de manera directa y efectiva a la problemática identificada en el transporte urbano. Este apartado presenta una visión general del enfoque de la solución, partiendo del análisis de los antecedentes y de las necesidades reales de los segmentos objetivo. Además, establece las bases conceptuales que guiarán la construcción del producto digital.
...
|------|---------|------------------------------|--------|--------------------------------------------------------------------------------------------------|--------------------------|
| US01 | E01 | Gestión de rutas y horarios  | Buscar rutas | Como pasajero, quiero buscar una ruta en el mapa para encontrar la mejor opción de transporte. | **Positivo:** Dado origen y destino válidos, cuando hago clic en “Buscar ruta”, entonces se muestran las rutas disponibles. <br> **Negativo:** Dado que los campos de origen y/o destino están vacíos, cuando intento presionar el botón “Buscar ruta”, entonces el botón permanece deshabilitado y no se ejecuta ninguna búsqueda. |
| US02 | E01 | Gestión de rutas y horarios  | Ver paraderos cercanos | Como pasajero, quiero ver los paraderos cercanos y las rutas que pasan por cada uno para elegir dónde tomar el bus. | **Positivo:** Dado que la app puede obtener mi ubicación actual o usar una ubicación por defecto,cuando ingreso a la pantalla “Paraderos cercanos”, entonces veo una lista de paraderos con su nombre, dirección, distancia y las rutas que pasan por cada uno. <br> **Negativo:** Dado que ocurre un error al obtener mi ubicación actual, cuando ingreso a “Paraderos cercanos”,entonces la app usa una ubicación por defecto y aun así se muestran paraderos y rutas disponibles. |
| US03 | E01 | Gestión de rutas y horarios  | Guardar rutas frecuentes | Como pasajero, quiero guardar mis rutas frecuentes para acceder a ellas rápidamente. | **Positivo:** Dado que selecciono una ruta, cuando presiono “Guardar como favorita”, entonces la ruta se guarda. <br> **Negativo:** Dado que la ruta ya está guardada, cuando intento guardarla otra vez, entonces aparece “Esta ruta ya está guardada”. |
| US04 | E02 | Monitoreo en tiempo real     | Ver ruta en Google Maps | Como pasajero, quiero abrir la ruta en Google Maps para visualizar el recorrido y las opciones de transporte disponibles. | **Positivo:** Dado que ingreso un origen y un destino válidos, cuando presiono el botón “Ver en Google Maps”, entonces se abre Google Maps mostrando la ruta correspondiente. <br> **Negativo:** Dado que ingreso un origen o destino que no corresponde a una dirección real, cuando presiono el botón “Ver en Google Maps”, entonces la aplicación igual abre Google Maps y muestra sugerencias para el origen y destino. |
| US05 | E02 | Monitoreo en tiempo real     | Notificaciones de retraso | Como pasajero, quiero recibir notificaciones si hay atraso para ajustar mi plan. | **Positivo:** Dado que el sistema registra una notificación de retraso para una de mis rutas, cuando se genera la notificación, entonces se muestra un toast emergente en la parte superior de la pantalla y la notificación aparece al inicio de la lista en la sección “Notificaciones”.   <br> **Negativo:** Dado que no tengo notificaciones registradas, cuando ingreso a la pantalla “Notificaciones”, entonces no se muestra ninguna tarjeta y veo un mensaje indicando que no tengo notificaciones. |
| US06 | E02 | Monitoreo en tiempo real     | Alertas de desvío | Como pasajero, quiero recibir alertas si un bus cambia de ruta. | **Positivo:** Dado que el sistema registra una alerta de desvío para una de mis rutas, cuando se genera la notificación, entonces se muestra un toast emergente en la parte superior y la alerta se agrega a la lista de notificaciones con su icono, mensaje y hora correspondiente.  <br> **Negativo:** Dado que tengo una notificación de desvío en la lista, cuando presiono el botón “×” de esa notificación, entonces la notificación se elimina de la lista y deja de mostrarse en pantalla. |
| US07 | E03 | Administración de flota      | Monitorear buses | Como administrador de flota, quiero monitorear mis buses en tiempo real. | **Positivo:** Dado que existen buses registrados en el sistema, cuando abro el panel de flota, entonces veo un resumen con el total de buses y la cantidad de buses activos, en mantenimiento e inactivos, además de una tabla con cada unidad y su estado.   <br> **Negativo:** Dado que no hay buses registrados (lista vacía), cuando abro el panel de flota, entonces la tabla no muestra filas de buses y no se ve información de estado por unidad. |
| US08 | E03 | Administración de flota      | Configurar alertas internas | Como administrador, quiero configurar alertas para incidentes de tráfico. | **Positivo:** Dado que completo los campos del formulario de “Agregar bus”, cuando presiono el botón “Guardar”, entonces el nuevo bus se agrega a la tabla de flota con un ID generado automáticamente.   <br> **Negativo:** Dado que estoy editando la información de un bus, cuando cierro el modal o presiono “Cancelar” sin guardar, entonces los cambios no se aplican y la información del bus se mantiene igual en la tabla. |
| US09 | E04 | Experiencia del usuario      | Registro de pasajero | Como nuevo pasajero, quiero registrarme para personalizar mis rutas. | **Positivo:** Dado que completo los campos, cuando presiono “Registrarse”, entonces se crea la cuenta. <br> **Negativo:** Dado campos vacíos, cuando intento registrarme, entonces aparece “Complete todos los campos”. |
| US10 | E04 | Experiencia del usuario      | Editar perfil | Como pasajero, quiero editar mis datos personales. | **Positivo:** Dado datos válidos, cuando guardo cambios, entonces el perfil se actualiza. <br> **Negativo:** Dado correo inválido, cuando guardo, entonces aparece “Correo no válido”. |
| US11 | E05 | Landing Page   | Información de la solución | Como visitante, quiero ver información de la solución para entender su utilidad. | **Positivo:**Dado que ingreso al home, cuando reviso el bloque principal y la sección “What Does BusTrack Offer?”, entonces veo un resumen claro de qué es BusTrack y qué funcionalidades ofrece. <br> **Negativo:** Dado que la página no carga correctamente (por ejemplo, fallo de internet), cuando ingreso al home, entonces no puedo ver el texto que describe la solución.|
| US12 | E05 | Landing Page   | Beneficios | Como visitante, quiero conocer los beneficios de la aplicación. | **Positivo:** Dado que accedo a la sección “Benefits of Using BusTrack”, cuando la reviso, entonces veo al menos tres beneficios explicados. <br> **Negativo:** Dado que por un problema de carga la sección de beneficios no se muestra, cuando navego por la landing, entonces no logro ver los beneficios de la aplicación.|
| US13 | E05 | Landing Page   | Misión y visión | Como visitante, quiero conocer la misión y visión de la empresa. | **Positivo:** Dado que ingreso a la sección “About Us”, cuando la reviso, entonces veo claramente la misión y la visión de BusTrack. <br> **Negativo:** Dado que la sección “About Us” no carga correctamente, cuando intento verla, entonces no puedo ver la misión ni la visión.|
| US14 | E04 | Experiencia del usuario | Acceso diferenciado | Como usuario, quiero elegir si ingreso como pasajero o empresa. | **Positivo:** Dado selección del tipo de usuario, cuando continúo, entonces accedo al módulo correspondiente. <br> **Negativo:** Dado que no selecciono ningún tipo de usuario, cuando intento continuar, entonces el botón “Continuar” permanece deshabilitado. |
| US15 | E03 | Administración de flota | Acceso para empresas |Como empresa, quiero acceder al módulo de monitoreo de flota para ver el estado de mis buses y gestionar alertas.  | **Positivo:** Dado que inicio sesión como empresa y el módulo de monitoreo carga correctamente, cuando abro “Monitoreo”, entonces veo el mapa y la lista de buses con su estado (a tiempo, con retraso o fuera de servicio). <br> **Negativo:** Dado un error de conexión con el servidor, cuando abro “Monitoreo”, entonces se muestra un aviso de “Error de conexión” con la opción de “Reintentar”. |
...
|------|---------|---------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------|
| TS01 | E01     | Endpoint: Buscar rutas                      | Como **Developer**, quiero implementar el endpoint `GET /api/routes/search` para obtener las rutas disponibles según origen y destino, de modo que el buscador del mapa funcione correctamente. | **Positivo:** Dado que el cliente envía una petición GET con parámetros `origin` y `destination` válidos, cuando el servicio procesa la solicitud, entonces retorna código **200** y un JSON con la lista de rutas encontradas. <br><br> **Negativo (parámetros inválidos):** Dado que faltan `origin` o `destination`, cuando se realiza la petición, entonces la API retorna código **400** con el mensaje `"Parámetros inválidos"`. <br><br> **Negativo (error interno):** Dado que ocurre un error inesperado en el servidor, cuando se procesa la solicitud, entonces la API retorna código **500** con un mensaje de error genérico. |
| TS02 | E01     | Endpoint: Rutas favoritas del pasajero      | Como **Developer**, quiero implementar los endpoints `GET /api/users/{userId}/favorites` y `POST /api/users/{userId}/favorites` para que la vista de “Rutas favoritas” pueda listar y guardar rutas frecuentes. | **Positivo (listar):** Dado que el usuario tiene rutas favoritas guardadas, cuando el frontend realiza un `GET /api/users/{userId}/favorites`, entonces la API retorna código **200** y un JSON con la lista de rutas. <br><br> **Positivo (agregar):** Dado que se envía una ruta que aún no está en favoritos, cuando el frontend ejecuta un `POST` con los datos de la ruta, entonces la API retorna código **201** y la ruta queda registrada. <br><br> **Negativo (duplicado):** Dado que la ruta ya existe en favoritos, cuando se intenta registrar nuevamente con `POST`, entonces la API retorna código **409** y el mensaje `"Ruta ya registrada en favoritos"`. |
...
En esta sección se presenta el diseño e implementación de la base de datos de BusTrack, la cual gestiona la información esencial de los usuarios, rutas, buses, paraderos y notificaciones. Este diseño garantiza el correcto funcionamiento de los módulos de la aplicación y su integración con las funcionalidades ya definidas en el modelo orientado a objetos.


### 4.8.1. Database Diagram

En primer lugar, se diseñó la base de datos mediante un diagrama Entidad–Relación (ER) utilizando la herramienta Lucidchart, seleccionada por su facilidad de uso y la disponibilidad de plantillas especializadas para modelado de bases de datos.

Descripción: El diagrama ER representa las entidades principales del sistema BusTrack, sus atributos y las relaciones entre usuarios, pasajeros, rutas, buses, paraderos, notificaciones y viajes.
...
Después de realizar el diseño conceptual, se implementó la base de datos en **MySQL Workbench**, lo que permitió visualizar el modelo físico generado a partir del script de creación. Esta herramienta facilitó la validación de las claves primarias, foráneas y relaciones N:M entre entidades.

Descripción: El diagrama físico generado en MySQL Workbench muestra la estructura final de las tablas, incluyendo claves primarias, claves externas y relaciones derivadas del modelo ER inicial.
...
A continuación, se describen las tablas implementadas en la base de datos, incluyendo sus relaciones.
...
Almacena la información asociada a los pasajeros.
...
- Relación **N:1** con `Users` (un usuario puede tener muchas notificaciones).
...
## 4. Tabla: `Route`
Representa las rutas disponibles en el sistema.
...
## 5. Tabla: `Stop`
Define los paraderos asociados a las rutas.
...
## 6. Tabla: `Route_Stop`
Representa la relación **N:M** entre las entidades `Route` y `Stop`.
...
## 7. Tabla: `Bus`
Almacena los datos de los buses registrados.
...
## 8. Tabla: `Driver`
Contiene los datos de los conductores asignados.

- Relación **1:1** con `Users`  
- Relación **N:1** con `Bus`
...
## 9. Tabla: `Travel`
Registra los viajes realizados por los pasajeros.

- Relación **N:1** con `Route`  
- Relación **N:1** con `Passenger`
